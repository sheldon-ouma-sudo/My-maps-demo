...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mapAdapter = MapsAdapter(this, userMaps, object : MapsAdapter.onCliickListener {
            override fun onItemClick(position: Int) {
                Log.i(TAG, "OnItemClick $position")
                //when user taps on a view in RV we navigate to a new activity
    ...
    private fun showAlertDialog() {
        ...
        val dialog =
            AlertDialog.Builder(this).setTitle("Map title").setView(mapFormView)
        ...
        dialog.getButton(DialogInterface.BUTTON_POSITIVE).setOnClickListener {
            ...
            if (title.trim().isEmpty()) {
                Toast.makeText(
                    this,
                    "Place must have a non-empty title and description",
                    Toast.LENGTH_SHORT
                ).show()
                return@setOnClickListener
            }
            // Navigte to create a map activity
            val intent = Intent(this@MainActivity, CreateNewMapsActivity::class.java)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode == REQUEST_CODE && resultCode == Activity.RESULT_OK) {
            //Get the new map data from the data
            val userMap = data?.getSerializableExtra(EXTRA_USER_MAP) as UserMap
            Log.i(TAG, "OnActivityResult with new map title${userMap.title}")
            userMaps.add(userMap)
            mapAdapter.notifyItemInserted(userMaps.size - 1)
            seriliazeMaps(this,userMaps )
        }
    ...
    private fun seriliazeMaps(context: Context, userMap: List<UserMap>){
    ...
    private fun getDataFile(context:Context) : File{
       Log.i(TAG, "Getting hte data file${context.filesDir}")
